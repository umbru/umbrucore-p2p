...
describe('Messages', function() {
  ...
  describe('@constructor', function() {
    it('sets properties correctly', function() {
      var network = umbrucore.Networks.defaultNetwork;
      var messages = new Messages({
        network: network,
        Block: umbrucore.Block,
        Transaction: umbrucore.Transaction
      });
      should.exist(messages.builder.commands);
      should.exist(messages.builder.constructors);
      messages.builder.constructors.Block.should.equal(umbrucore.Block);
      messages.builder.constructors.Transaction.should.equal(umbrucore.Transaction);
    ...
    it('network should be unique for each set of messages', function() {
      ...
      messages.network.should.deep.equal(umbrucore.Networks.livenet);
      messages2.network.should.deep.equal(umbrucore.Networks.testnet);
      var message1 = messages.Version();
      message1.network.should.deep.equal(umbrucore.Networks.livenet);
      var message2 = messages2.Version();
      message2.network.should.deep.equal(umbrucore.Networks.testnet);
    });
  });
  ...
  describe('Default Network', function() {
    ...
    Object.keys(messages.builder.commandsMap).forEach(function(command) {
      ...
      it(name, function() {
        var message = messages[name]();
        message.network.should.deep.equal(umbrucore.Networks.defaultNetwork);
      });
    });
